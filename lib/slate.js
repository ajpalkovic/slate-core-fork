...
var Editor = function () {
  ...
  function Editor() {
    ...
    this.tmp = {
      ...
      nativeOperationQueue: immutable.List()
    };
  ...
  createClass(Editor, [{
    ...
    value: function applyOperation(operation) {
      ...
      if (this.tmp.asNativeOperation) {
        this.tmp.nativeOperationQueue = this.tmp.nativeOperationQueue.push(operation);
        return;
      }
    ...
    key: 'flushQueuedNativeOperations',
    value: function flushQueuedNativeOperations() {
      var _this2 = this;

      if (!this.tmp.nativeOperationQueue.count()) return;

      this.tmp.nativeOperationQueue.forEach(function (operation) {
        _this2.applyOperation(operation);
      });

      this.tmp.nativeOperationQueue = immutable.List();
      this.flush();
    }
    ...
    value: function registerCommand(type) {
      ...
      var method = function method() {
        ...
        return _this3.command.apply(_this3, [type].concat(args));
      };
    ...
    value: function registerQuery(type) {
      ...
      var method = function method() {
        ...
        return _this4.query.apply(_this4, [type].concat(args));
      };
      controller[type] = method;
      method.__query = true;
      return controller;
    ...
    key: 'asNativeOperation',
    value: function asNativeOperation(fn) {
      var controller = this.controller;

      var value = this.tmp.asNativeOperation;
    ...
    value: function applyOperations(operations) {
      ...
      operations.forEach(function (op) {
        return _this5.applyOperation(op);
      });
      return this.controller;
    }
  }, {
